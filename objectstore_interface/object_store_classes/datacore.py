...
class DataCore(ObjectStore):
    ...
    async def get_store(self, request: Request, password: str = None) -> dict:
        ...
        # Update date fields to viewable format and check if they're expired/expiring.
            ...
            lifepoint = datetime.strptime(json_access_key['lifepoint'].split(']')[0], '[%a, %d %b %Y %H:%M:%S %Z') # This converts the expiry date to a human readable format
            ...
            access_key['owner'] = json_access_key['x_owner_meta'] # This generates the dictionary used by the template.
            ...
            if access_key['description'] != "JASMIN account auth access key" and access_key['description'] != 'JASMIN account auth access key S3':
                ...
                if not expired:
                    access_keys.append(access_key) # Append unexpired access keys to a list to be returned
                else:
                    expired_access_keys.append(access_key) # List for expired access keys in case there are no valid access keys.
        ...
        if access_keys == [] and expired_access_keys != []:
            ...
            access_keys.append(expired_access_keys[0]) # Grab the most recently expired access key if there are no valid access keys
    ...
    async def get_access_key(self, password, request: Request):
        ...
        response = r.get(f"http://{self.location}:81/.TOKEN/?format=json", auth=HTTPBasicAuth(request.session["token"]["userinfo"]["preferred_username"], password)) # Use users jasmin password to authenticate the first time.
        if response.status_code != 200:
                return self._return_error(response)
        auth_access_keys = [ak for ak in response.json() if ak['x_custom_meta_source'] == "JASMIN account auth access key"] # Find all access keys
        s3_auth_access_keys = [ak for ak in response.json() if ak['x_custom_meta_source'] == "JASMIN account auth access key S3"] # Find all s3 access keys
    ...
    async def create_key(self, description, expires):
        secret_key = ''.join(random.choices(string.ascii_letters + string.digits + string.punctuation, k=64)) # Generate secret key
        headers = {
                'X-User-Secret-Key-Meta': secret_key,
    ...
    async def delete_policy(self, bucket, policy):
        ...
        bucket_policy_str = json.dumps(bucket_policy) # Converts the policy list back to a 
        jasmin_client.put_bucket_policy(Bucket=bucket, Policy=bucket_policy_str)
